...
Output structure under surfacesimilarity_main/:
...
BASE_DIR  = os.path.join(REPO_ROOT, "surfacesimilarity_main")
CDN_BASE  = "https://cdn.jsdelivr.net/gh/pagrass/pilot1-asset-data@latest/surfacesimilarity_main/crypto/current/"
...
def git_commit_and_push(repo_root, paths_to_add, branch="main"):
    ...
    try:
        ...
        msg = f"surfacesimilarity_main: update crypto data {datetime.now().strftime('%Y-%m-%d %H:%M')}"
...
def main():
    ...
    print("=" * 50)
    print("FETCHING CRYPTO PRICE DATA (surfacesimilarity_main)")
    print("=" * 50)
    ...
    for slug in CRYPTOS.values():
        print(f"  curl -s https://purge.jsdelivr.net/gh/pagrass/pilot1-asset-data@latest/surfacesimilarity_main/crypto/current/{slug}_365d.json")
