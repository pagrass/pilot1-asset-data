#!/usr/bin/env python3
"""
Fetch crypto data for the Investment Decisions + Persistence study (Wave 1).

Cryptos (slate inherited from the structural-similarity pilot, 2026-08-24):
...
Output structure under decisions_pilot/:
...
CDN_BASE  = "https://cdn.jsdelivr.net/gh/pagrass/pilot1-asset-data@latest/decisions_pilot/crypto/current/"
...
def git_commit_and_push(repo_root, paths_to_add, branch="main"):
    ...
    try:
        ...
        msg = f"decisions_pilot: update crypto data {datetime.now().strftime('%Y-%m-%d %H:%M')}"
...
def main():
    ...
    print("=" * 50)
    print("FETCHING CRYPTO PRICE DATA (decisions_pilot)")
    print("=" * 50)
    ...
    for slug in CRYPTOS.values():
        print(f"  curl -s https://purge.jsdelivr.net/gh/pagrass/pilot1-asset-data@latest/decisions_pilot/crypto/current/{slug}_365d.json")
