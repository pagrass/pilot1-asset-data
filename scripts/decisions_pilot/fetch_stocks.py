#!/usr/bin/env python3
"""
Fetch stock data for the Investment Decisions + Persistence study — Wave 2 (pilot).

Slate (inherited from the structural-similarity pilot, 2026-08-21; broad, roughly even return ladder, all comfortably
GAAP-profitable — % profit growth needs a positive base; ZS excluded for
TTM losses, TER/AMKR/MU/ORCL excluded for ludicrous charts):
  Anchors (familiar):  MSFT (-5%), CSCO (+64%)
  Negative:            INTU (-48%), NOW (-26%)
  Positive:            FFIV (+20%), AKAM (+44%)
Repeats: MSFT/CSCO (baseline + surface), NOW (baseline), FFIV/AKAM (surface);
INTU is the one newcomer (deep-negative slot).
...
Output: decisions_pilot/stock/current/ + decisions_pilot/stock/runs/run_YYYY-MM-DD/
"""
...
BASE_DIR  = os.path.join(REPO_ROOT, "decisions_pilot")
...
PEER_MKTCAP_FLOOR = int(os.environ.get("PEER_MKTCAP_FLOOR", "10000"))
...
def fetch_price_data(ticker, max_retries=MAX_RETRIES):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # Truncate to the last COMPLETED trading day (mirrors fetch_crypto's
            # completed-UTC-day rule): during/after a live session Yahoo appends a
            # bar for the current ET date whose "close" is just the latest quote —
            # dropping it makes the fetch deterministic and keeps the displayed
            # "Latest closing price" an actual close.
...
def fetch_fundamentals_yf(ticker):
    """Fetch market cap, dividend yield, P/B, P/E and TTM net income from yfinance."""
    ...
    try:
        ...
        return {
            ...
            "netprofit_m":   netprofit_m,
            "netprofit_src": netprofit_src,
        }
    except Exception as e:
...
def pb_to_valuation_tertile(pctile):
    """Valuation classification: Low (<=33rd), Mid (34-66th), High (>=67th)."""
    if pctile is None:
        return None
    if pctile <= 33:
        return "Low"
...
def git_commit_and_push(repo_root, paths_to_add, branch="main"):
    ...
    try:
        ...
        msg = f"decisions_pilot: update stock data {datetime.now().strftime('%Y-%m-%d %H:%M')}"
...
def main():
    ...
    for sym in STOCKS:
        ...
        # Market cap, dividend yield, current P/B, P/E and TTM net profit from yfinance
        raw = fetch_fundamentals_yf(sym)
        pb_yahoo = pe_yahoo = netprofit_m = netprofit_src = None
        if raw is not None:
        ...
        if netprofit_m is None:
            summary["errors"].append({"symbol": sym, "type": "netprofit"})
        ...
        fundamentals[sym] = {
            ...
            "div_y":             div_pct,       # kept in the data; NOT displayed in the panel
            "netprofit":         netprofit_m,   # TTM GAAP net income, $M (displayed as $X.XB)
            "netprofit_src":     netprofit_src, # 'quarterly_sum' (preferred) or 'netIncomeToCommon'
            "valuation":         valuation,
            "sector":            SECTOR_LABEL[sym],
        }
        np_disp = f"${netprofit_m/1000:.1f}B" if netprofit_m is not None else "–"
        print(f"  ✅ {sym}: mcap={mc_millions}M, PB={pb_yahoo} ({pb_pctile}th pctile {valuation}), "
              f"PE={pe_yahoo}, div={div_pct}%, netprofit={np_disp} [{netprofit_src}]")
        time.sleep(SLEEP_SEC)
    ...
    print(f"  https://cdn.jsdelivr.net/gh/pagrass/pilot1-asset-data@latest/decisions_pilot/stock/current/fundamentals.json")
